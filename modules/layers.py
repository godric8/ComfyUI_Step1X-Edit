...
class RMSNorm(torch.nn.Module):
    def __init__(self, dim: int):
        super().__init__()
        self.scale = nn.Parameter(torch.ones(dim))  # 可学习缩放参数[2,3,7](@ref)
        self.eps = 1e-6  # 数值稳定系数[3,6,8](@ref)

    @staticmethod
    def rms_norm(x: Tensor, weight: Tensor, eps: float):
        """纯PyTorch实现的RMSNorm核心计算[3,4,6,8](@ref)"""
        x_dtype = x.dtype
        # 计算均方根倒数 (RRMS) = 1 / sqrt(mean(x^2) + eps)
        rrms = torch.rsqrt(torch.mean(x.float()**2, dim=-1, keepdim=True) + eps)
        # 归一化并应用缩放权重
        return (x.float() * rrms).to(dtype=x_dtype) * weight
    ...
    def forward(self, x: Tensor):
        if TRITON_AVAILABLE:
            ...
            # 回退到纯PyTorch实现
            return self.rms_norm(x, self.scale.to(x.dtype), self.eps)
